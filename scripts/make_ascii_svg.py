...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "vbv18-ascii.svg")
...
CONTRAST = 1.15
BRIGHTNESS = 1.0
GAMMA = 0.95         # >1 brightens mids -> face lands in sparser chars
SHARPEN = True
...
# Character cells are taller than they are wide, so compensate for that.

ascii_ratio = CELL_W / CELL_H

new_height = int(
    COLS * im.height / im.width * ascii_ratio
)
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">vbv18@github: ~$ ./portrait.sh</text>')
...
for ry, line in enumerate(rows_txt):
    ...
    text = (f'<text xml:space="preserve" x="{PAD}" y="{y:.2f}" fill="{INK}" '
    ...
    parts.append(
        f'<rect y="{row_y+1:.1f}" width="{CELL_W}" height="{CELL_H}" fill="{CURSOR}" opacity="0">'
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'vbv18@github:~$ whoami <tspan fill="{INK}">Vaibhav Garg</tspan></text>')
